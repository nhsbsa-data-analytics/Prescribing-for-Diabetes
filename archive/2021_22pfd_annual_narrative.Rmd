---
title: "Prescribing for diabetes"
...
Additionally sensors for continuous glucose monitoring devices are not included though are being being considered for future publications following changes to NICE guidelines. On inclusion of this data continuous glucose monitoring devices will continue to be excluded as these typically aren't prescribed.
...
**Table 1: The proportion of items for which an NHS number was recorded for listed BNF sections 2015/16 to 2021/22**
...
pfd_national_overall %>%
  filter(DRUG_TYPE == "Drugs used in diabetes") %>%
  group_by(FINANCIAL_YEAR) %>% 
  summarise(`Prescription items` = sum(ITEM_COUNT),
...
There were 60.3 million items for drugs used in diabetes prescribed in 2021/22. This was a 21.2% increase from 49.7 million in 2015/16, and a 4.11% increase from 57.9 million items in 2020/21. The increase in items between 2020/21 and 2021/22 was larger than seen in previous yearly increases, which between 2015/16 and 2020/21 was an average of 3.10%. 

The number of identified patients that received prescribing for drugs used in diabetes was 3.20 million in 2021/22. This was an 18.2% increase from 2.70 million identified patients in 2015/16, and a 4.95% increase from 3.05 million in 2020/21. The increase in identified patients between 2020/21 and 2021/22 was the largest increase observed in the period, with yearly increases between 2015/16 and 2020/21 averaging 2.42%.
...
pfd_national_overall %>%
  filter(DRUG_TYPE == "Drugs used in diabetes") %>%
  group_by(FINANCIAL_YEAR) %>% 
  summarise(ITEM_PAY_DR_NIC = sum(ITEM_PAY_DR_NIC)) %>% 
...
pfd_national_overall %>%
  group_by(FINANCIAL_YEAR, DRUG_TYPE) %>% 

  summarise(ITEM_COUNT = sum(ITEM_COUNT),
            ITEM_PAY_DR_NIC = sum(ITEM_PAY_DR_NIC)) %>% 
  group_by(FINANCIAL_YEAR) %>% 
  mutate(`Proportion of items` = ITEM_COUNT / sum(ITEM_COUNT) * 100,
         `Proportion of costs` = ITEM_PAY_DR_NIC / sum(ITEM_PAY_DR_NIC) * 100) %>% 
  filter(DRUG_TYPE == "Drugs used in diabetes") %>% 
  select(-ITEM_COUNT, -ITEM_PAY_DR_NIC) %>% 
  pivot_longer(cols = c(`Proportion of items`, `Proportion of costs`),
               names_to = "measure",
               values_to = "value") %>% 
...
pfd_paragraph_data %>%
  group_by(`Financial Year`,`BNF Paragraph Name`) %>% 
  summarise(`Total Net Ingredient Cost (GBP)` = signif(
    sum(
      `Total Net Ingredient Cost (GBP)`
      )
    ,3)) %>% 
  pivot_longer(cols = c(`Total Net Ingredient Cost (GBP)`),
               names_to = "measure",
               values_to = "value") %>% 
  group_chart_hc(
    x = `Financial Year`,
...
pfd_national_overall %>%
  filter(IDENTIFIED_FLAG == "Y", DRUG_TYPE == "Drugs used in diabetes") %>%
  
  mutate(ITEMS_PER_PAT = signif(ITEM_COUNT  / PATIENT_COUNT, 3)) %>%
...
pfd_gender_data%>%
  ...
  summarise(`Total Identified Patients` = signif(
    sum(
      `Total Identified Patients`
      )
    ,3)) %>%
  
  pivot_longer(cols = c(`Total Identified Patients`),
               names_to = "measure",
               values_to = "value") %>% 
  group_chart_hc(
    x = `Financial Year`,
...
You can complete a [short survey about this publication](https://wh1.snapsurveys.com/s.asp?k=160317152537) to help us improve the Official Statistics that we produce. All responses will remain anonymous, and individuals will not be identifiable in any report that we produce.
